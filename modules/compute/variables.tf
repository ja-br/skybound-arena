...
variable "desired_count" {
  description = "Initial/floor task count. Autoscaling owns it at runtime (the service ignores drift on it), so this is only the count at first apply."
  type        = number
  default     = 1
}

# --- Autoscaling (target-tracking on ECS CPU) --------------------------------
variable "min_capacity" {
  description = "Minimum ECS task count the autoscaler will hold."
  type        = number
  default     = 1
}

variable "max_capacity" {
  description = "Maximum ECS task count the autoscaler will scale out to."
  type        = number
  default     = 4
}

variable "cpu_target_value" {
  description = "Target average ECS service CPU %. Autoscaler adds/removes tasks to hold this."
  type        = number
  default     = 60
}

variable "scale_out_cooldown" {
  description = "Seconds to wait after a scale-out before another. Short: react fast to a spike."
  type        = number
  default     = 60
}

variable "scale_in_cooldown" {
  description = "Seconds to wait after a scale-in before another. Long: scale in slowly to avoid flapping."
  type        = number
  default     = 300
}
